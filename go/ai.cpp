    ...
    : QMainWindow{parent}
{
    ...
    Btn11->setText("悔棋");
    Btn11->setFixedSize(60,30);
    Btn11->move(0,630);
    connect(Btn11,&QPushButton::clicked,[=](){
        emit this->airegret();
    });
    connect(this,&ai::airegret,[=](){
        game->airegret();
        update();
    });
}
...
void ai::paintEvent(QPaintEvent *event)
{
    ...
    for (int i =0;i<BOARD_GRAD_SIZE + 1;i++)
    {
        ...
        painter.drawLine(MARGIN,MARGIN + BLOCK_SIZE * i,
                         size().width() - MARGIN, MARGIN + BLOCK_SIZE * i);   //从上到下，第i+条竖线
    }
...
void ai::mouseMoveEvent(QMouseEvent *event)
{
    int x = event->x();
    int y = event->y();
...
void ai::mouseReleaseEvent(QMouseEvent *event)
{
    if(selectPos ==false)
    {
        return;
